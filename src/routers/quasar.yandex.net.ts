...
function mapStationInfoToStationSyncInfoResponseConfig(info: StationInfo): unknown {
    ...
    return {
        ...
        system_config: {
            ...
            appmetrikaReportEnvironment: env,
            audioInput: {
                model_storage: {
...
export function registerQuasarYandexNetRouter(app: Application, deps: QuasarRouterDeps): void {
    ...
    router.post("/update_device_state", async (req, res) => {
        try {
            const duid = String(req.query['device_id'])
            if (!duid) {
                throw new Error('No device_id present in query')
            }
            const platform = String(req.query['platform']) ?? 'unknown'

            await deps.infoProvider.updateNetworkInfo(duid, platform, req.body)
    ...
    router.get("/glagol/device_list", async (req, res) => {
        try {
            ...
            res.json({
                devices: stationInfos.map(info => ({
                    activation_code: Math.floor(Math.random() * 1000000000),
    ...
    router.get("/get_sync_info", async (req, res) => {
